...
export const createProductController = async (req, res) => {
  try {
    ...
    const products = new product({ ...req.fields, description: name });
    if (photo) {
...
export const updateProductController = async (req, res) => {
  try {
    ...
    const products = await product.findByIdAndUpdate(req.params.pid,{ ...req.fields, description: name },{ new: true });
    if (photo) {
      products.photo.data = fs.readFileSync(photo.path);
      products.photo.contentType = photo.type;
    }
...
export const searchProductController = async (req, res) => {
  try {
    ...
    const results = await product.find({$or: [{ name: { $regex: keyword, $options: "i" } },{ description: { $regex: keyword, $options: "i" } },],}).select("-photo");
    res.json(results);
  } catch (error) {
    console.log(error);
    res.status(400).send({
...
export const productCategoryController = async (req, res) => {
  try {
    const categoryObj = await category.findOne({ slug: req.params.slug });
    const products = await product.find({ categoryObj }).populate("category");
    res.status(200).send({
      success: true,
      categoryObj,
      products,
